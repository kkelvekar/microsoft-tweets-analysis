...
# count the word frequencies and save them to a csv file
word_frequencies = Counter(all_words.split())
word_freq_df = pd.DataFrame.from_dict(word_frequencies, orient='index').reset_index()
word_freq_df.columns = ['Word', 'Frequency']
word_freq_df.to_csv('data/exploratory_data_analysis/word_frequencies.csv', index=False)
...
# save the line plot data to a csv file
line_plot_data = df['product'].value_counts().sort_index().reset_index()
line_plot_data.columns = ['Product', 'Count']
line_plot_data.to_csv('data/exploratory_data_analysis/line_plot_data.csv', index=False)
...
# count the hashtag frequencies and save them to a csv file
hashtag_frequencies = Counter(all_hashtags)
hashtag_freq_df = pd.DataFrame.from_dict(hashtag_frequencies, orient='index').reset_index()
hashtag_freq_df.columns = ['Hashtag', 'Frequency']
hashtag_freq_df.to_csv('data/exploratory_data_analysis/hashtag_frequencies.csv', index=False)
